        ...
        <div className="animate-fadeInUp delay-200">
          <Card className="glass-card p-6 xs:p-8 lg:p-12 rounded-[2rem] sm:rounded-[3rem] border-white/10 shadow-[0_0_60px_rgba(0,0,0,0.5)] relative overflow-hidden">
            ...
            <CardContent className="space-y-8 p-0">
              ...
              <div className="glass bg-white/5 border border-white/10 rounded-3xl p-6 relative group">
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/5">
                  <h4 className="font-black text-[10px] uppercase tracking-[0.2em] text-primary">Secure Entry Nodes</h4>
                  <Zap className="w-4 h-4 text-primary animate-pulse" />
                </div>
                <div className="space-y-4">
                  {(['student', 'instructor', 'admin'] as const).map((role) => (
                    <div key={role} className="flex items-center justify-between group/role p-2 hover:bg-white/5 rounded-xl transition-all">
                      <div>
                        <span className="font-black text-xs uppercase tracking-widest text-foreground/80">{role}</span>
                        <p className="text-[10px] font-medium text-muted-foreground mt-0.5">{testCredentials[role].email}</p>
                      </div>
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => fillCredentials(role)}
                        className="text-[10px] font-black uppercase h-8 px-4 rounded-lg glass border-white/10 hover:bg-primary hover:text-white transition-all"
                      >
                        Authorize
                      </Button>
                    </div>
              ...
              <form onSubmit={onSubmit} className="space-y-6">
                <div className="space-y-3">
                  <Label htmlFor="email" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-2">Secure Email</Label>
                  <Input
                    id="email"
                    type="email"
                    value={email}
                ...
                <div className="space-y-3">
                  <Label htmlFor="password" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-2">Access Key</Label>
                  <Input
                    id="password"
                    type="password"
                    value={password}
              ...
              <div className="text-center text-sm font-bold pt-4">
                ...
                <Link href="/sign-up" className="text-primary hover:underline font-black uppercase tracking-widest text-xs ml-2">
                  Initiate Account
                </Link>
              </div>
            </CardContent>
